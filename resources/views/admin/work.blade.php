...
<section class="home-section">
  ...
  <a href="{{ route('work.create') }}"><button class="button">
  ...
  <table class="table">
    ...
    <tbody>
      <tr>
        ...
        <td>{{ $work->id}}</td>
        <td>{{ $work->name}}</td>
        <td><img src="{{ asset( $work->image )}}" style="width: 100px; height: 100px;" alt="Img" /></td>
        <td>{{ $work->description}}</td>
        <td>{{ $work->link}}</td>
        <td>{{ $work->github}}</td>
        <td>
          <a href="{{ url('/work/'.$work->id.'/edit') }}"><button class="action">Edit</button>
          <button class="action delete-btn" data-url="{{ url('/work/'.$work->id.'/delete') }}">Delete</button>
        </td>
      </tr>
      @endforeach
    </tbody>
