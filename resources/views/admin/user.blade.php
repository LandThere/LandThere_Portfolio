  ...
  <table class="table">
    ...
    <tbody>
      <tr>
        ...
        <td>{{ $user->name}}</td>
        <td>{{ $user->username}}</td>
        <td><img src="{{ asset( $user->image )}}" style="width: 100px; height: 100px;" alt="Img" /></td>
        <td>
          <a href="{{ url('/users/'.$user->id.'/edit') }}"><button class="action">Edit</button>
          <button class="action delete-btn" data-url="{{ url('/users/'.$user->id.'/delete') }}">Delete</button>
        </td>
      </tr>
      @endforeach
    </tbody>
