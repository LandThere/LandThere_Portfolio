...
<section class="home-section">
  ...
  <a href="{{ route('timeline.create') }}"><button class="button">
  ...
  <table class="table">
    ...
    <tbody>
      <tr>
        ...
        <td>{{ $timeline->id}}</td>
        <td>{{ $timeline->course}}</td>
        <td>{{ $timeline->year}}</td>
        <td>{{ $timeline->school}}</td>
        <td><img src="{{ asset( $timeline->image )}}" style="width: 50px; height: 50px;" alt="Img" /></td>
        <td>
          <a href="{{ url('/timeline/'.$timeline->id.'/edit') }}"><button class="action">Edit</button>
          <button class="action delete-btn" data-url="{{ url('/timeline/'.$timeline->id.'/delete') }}">Delete</button>
        </td>
      </tr>
      @endforeach
    </tbody>
